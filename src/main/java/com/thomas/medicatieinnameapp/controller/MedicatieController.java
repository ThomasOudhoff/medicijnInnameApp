...
public class MedicatieController {
    ...
    public ResponseEntity<Void> uploadBijsluiter(@PathVariable Long id,
        ...
        try {
            ...
            medicatieService.saveBijsluiterFoto(
                    id,
                    file.getBytes(),
                    file.getContentType(),
    ...
    public ResponseEntity<byte[]> downloadBijsluiter(@PathVariable Long id) {
        byte[] data = medicatieService.getBijsluiterFoto(id); // gooit 404 als er niets is

        var opt = medicatieService.getBijsluiter(id);
        String ct = opt.isPresent() && opt.get().getContentType() != null && !opt.get().getContentType().isBlank()
                ? opt.get().getContentType()
                : "application/octet-stream";

        MediaType mediaType;
        try { mediaType = MediaType.parseMediaType(ct); }
        catch (Exception ex) { mediaType = MediaType.APPLICATION_OCTET_STREAM; }

        String filename = opt.isPresent() && opt.get().getFilename() != null && !opt.get().getFilename().isBlank()
                ? opt.get().getFilename().replace("\"","")
                : "bijsluiter-" + id;
